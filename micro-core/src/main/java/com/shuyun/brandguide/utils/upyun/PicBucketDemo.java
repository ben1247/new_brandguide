...
public class PicBucketDemo {
	...
	// 运行前先设置好以下三个参数
	private static final String BUCKET_NAME = "brand-guide";
	private static final String OPERATOR_NAME = "bgadmin";
	private static final String OPERATOR_PWD = "brandguideshuyun";
	...
	/** 上传到upyun的图片名 */
	private static final String PIC_NAME = "1242140302.p12";

	/** 本地待上传的测试文件 */
	private static final String SAMPLE_PIC_FILE = System.getProperty("user.dir") + "/micro-files/1242140302.p12";
...
//		upyun.setFileSecret("bac");
	...
	public static void testGmkerl() throws IOException {
		...
		// 要传到upyun后的文件路径
		String filePath = DIR_ROOT + new Date().getTime()+".jpg";
		...
		params.put(UpYun.PARAMS.KEY_X_GMKERL_VALUE.getValue(), "200x200");

//		// 设置缩略图类型，必须搭配缩略图参数值（KEY_VALUE）使用，否则无效
//		params.put(UpYun.PARAMS.KEY_X_GMKERL_TYPE.getValue(), UpYun.PARAMS.VALUE_FIX_WIDTH.getValue());
//		// 设置缩略图参数值，必须搭配缩略图类型（KEY_TYPE）使用，否则无效
//		params.put(UpYun.PARAMS.KEY_X_GMKERL_VALUE.getValue(), "900");
